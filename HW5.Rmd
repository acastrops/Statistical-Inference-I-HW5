...
# a, c
Nonlinear <- function(n, b0, b1, b2, func, bool = FALSE){
  ...
  if (func == "exponential"){
    ...
    exp.ey <- b0 + b1*exp(x1)
    exp.lm <- lm(exp.y ~ x1)
    exp.reg.stats <- reg.stats(exp.lm)
    exp.reg.tests <- reg.tests(exp.reg.stats[1], exp.reg.stats[2], exp.reg.stats[4], exp.ey, list(b0,b1))
    output(exp.reg.tests[1], exp.reg.tests[2], exp.reg.tests[3], exp.reg.stats[3], var,exp.lm, bool)
  }
  ...
  if(func == "polynomial"){
    ...
    pol.reg.stats <- reg.stats(pol.lm)
    pol.reg.tests <- reg.tests(pol.reg.stats[1], pol.reg.stats[2], pol.reg.stats[4], pol.ey, list(b0, b1, b2))
    output(pol.reg.tests[1], pol.reg.tests[2], pol.reg.tests[3], pol.reg.stats[3], var, pol.lm, bool)
  }
  ...
  if(func == "sinusoidal"){
    ...
    sin.ey <-b0 + b1*sin(x1)
    sin.lm <- lm(sin.y ~ x1)
    sin.reg.stats <- reg.stats(sin.lm)
    sin.reg.tests <- reg.tests(sin.reg.stats[1], sin.reg.stats[2], sin.reg.stats[4], sin.ey, list(b0,b1))
    output(sin.reg.tests[1], sin.reg.tests[2], sin.reg.tests[3], sin.reg.stats[3], var, sin.lm, bool)
  }
  ...
  if(func == "normal"){
    ...
    norm.reg.stats <- reg.stats(norm.lm)
    norm.reg.tests <- reg.tests(norm.reg.stats[1], norm.reg.stats[2], norm.reg.stats[4], norm.ey, list(b0,b1))
    output(norm.reg.tests[1], norm.reg.tests[2], norm.reg.tests[3], norm.reg.stats[3], var, norm.lm, bool)
  }
...
reg.tests <- function(yhat, confint, pvalue, ey, betaslist)  {
  ...
  # pvalue test
  # create a list of lists with coefficient name and boolean of if pvalue significant
...
output <- function(yhat.coverage, beta.pass, coefficient.df, mse, var, lm, bool){
  ...
  if (!all(coefficient.df$X2)){
    print(paste("The following coefficients were not significant", coefficient.df[coefficient.df$X2 == FALSE]))
  }
  else {
...
for (i in 1:length(samp)){
  for (j in 1:length(size)){
    ...
    Nonlinear(size[j], b0,b1,b2,samp[i])
  }
}
```
Exponential
...
n <- sample(20:5000, 1)
i <- sample(1:4, 1)
Nonlinear(n, b0,b1,b2,samp[i],TRUE)

```

After testing ourselves 25 times we found that we were accurate 22 times out of 25.  Resulting in 88% accuracy for testing our groups
...
nonnormal_functions <- c("poisson","gamma")

Nonnormal <- function(n, b0, b1, func, bool = FALSE){
  b0 <- 10
  b1 <- 2
  eps_alpha <- 2 #Shape
  eps_beta <- 1/4 #Scale
  ...
  # poisson
  if (func == "poisson"){
    eps <- rpois(n, lambda = 5)
    var <- var(eps)
    pois.y <- b0 - b1*(x) + eps
    pois.ey <- b0 - b1*(x)
    pois.lm <- lm(pois.y ~ x)
    pois.reg.stats <- reg.stats(pois.lm)
    pois.reg.tests <- reg.tests(pois.reg.stats[1], pois.reg.stats[2], pois.reg.stats[4], pois.ey, list(b0,b1))
    output(pois.reg.tests[1], pois.reg.tests[2], pois.reg.tests[3], pois.reg.stats[3], var,pois.lm, bool)
  }
  ...
  if(func == "gamma"){
    ...
    gamma.reg.stats <- reg.stats(gamma.lm)
    gamma.reg.tests <- reg.tests(gamma.reg.stats[1], gamma.reg.stats[2], gamma.reg.stats[4], gamma.ey, list(b0,b1))
    output(gamma.reg.tests[1], gamma.reg.tests[2], gamma.reg.tests[3], gamma.reg.stats[3], var,gamma.lm, bool)
  }
...
for (i in 1:length(nonnormal_functions)){
  for (j in 1:length(size)){
    ...
    Nonnormal(size[j], b0,b1,nonnormal_functions[i])
  }
}
```
...
n <- sample(20:5000, 1)
i <- sample(1:2, 1)
Nonnormal(n, b0,b1,nonnormal_functions[i],TRUE)

# to-do add normal case!!
...
Hetero <- function(n, b0, b1, func, bool = FALSE){
  # Melissa gave us
  b0 <- 10
  b1 <- 10
  x1 <- runif(n, 0, 10)
  ...
  if (func == 'sqrt'){
    eps <- rnorm(n, sd = 2*sqrt(x1))
    var<-var(eps)
    sqrt.y <- b0 + b1*(x1) + eps
    sqrt.ey <- b0 + b1*(x1)
    sqrt.lm <- lm(sqrt.y ~ x1)
    sqrt.reg.stats <- reg.stats(sqrt.lm)
    sqrt.reg.tests <- reg.tests(sqrt.reg.stats[1], sqrt.reg.stats[2], sqrt.reg.stats[4], sqrt.ey, list(b0,b1))
    output(sqrt.reg.tests[1], sqrt.reg.tests[2], sqrt.reg.tests[3], sqrt.reg.stats[3], var, sqrt.lm, bool)
  }
  
  #poly
  
  if (func == 'poly'){
    eps <- rnorm(n, sd = 0.5*x1^2)
    var<-var(eps)
    poly.y <- b0 + b1*(x1) + eps
    poly.ey <- b0 + b1*(x1)
    poly.lm <- lm(poly.y ~ x1)
    poly.reg.stats <- reg.stats(poly.lm)
    poly.reg.tests <- reg.tests(poly.reg.stats[1], poly.reg.stats[2], poly.reg.stats[4], poly.ey, list(b0,b1))
    output(poly.reg.tests[1], poly.reg.tests[2], poly.reg.tests[3], poly.reg.stats[3], var, poly.lm, bool)
  }
...
for (i in 1:length(samp.hetero)){
  for (j in 1:length(size)){
    ...
    Hetero(size[j], b0,b1,samp.hetero[i])
  }
}
```
...
n <- sample(20:5000, 1)
i <- sample(1:2, 1)
Hetero(n, b0,b1,samp.hetero[i],TRUE)
```
...
x1 <- runif(n, 0, 10)
x2_q5 <- x1 + rnorm(n) #x2 is a linear combination of x1 and some number

eps <- rnorm(n, sd = sigma) 
y <- b0 + b1 * x1 + b2 * x2_q5 + eps


## Testing for colinearity
cor(x1, x2_q5) # Showing there is correlation
## A high kappa indicates trouble
test_colinearity <- model.matrix(~ x1 + x2_q5)        
kappa(mm12)                          
```
