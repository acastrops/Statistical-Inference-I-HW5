...
Nonlinear <- function(n, b0, b1, b2){
  ...
  output(exp.reg.tests[1], exp.reg.tests[2], exp.reg.tests[3], exp.reg.stats[3], var)
...
reg.stats <- function(lm){
  ...
  confint <- confint(lm)
  mse <- mean(residuals(lm)^2)
  pvalue <- list(rownames(summary(lm)$coefficients), summary(lm)$coefficients[,4])
...
reg.tests <- function(yhat, confint, pvalue, ey, betaslist)  {
  ...
  for (p in 1:length(pvalue)){
    pval.test[p] <- list(pvalue[[1]][[1]][p], (pvalue[[1]][[2]][p] <= 0.05))
  }
  
  return(list(yhat.coverage, beta.pass, pval.test))
}

output <- function(yhat.coverage, beta.pass, pval.test, mse, var){
  # is beta in confint?
  print("Betas are within their confidence interval: ")
  print(beta.pass[[1]])
  
  # is EYi in confint for Yhat?
  print("How often does the EYi fall within the Yhat confidence interval?: ", yhat.coverage, " % of the time")
  
  # How close is MSE to sigma^2? Later this will be represented as a histogram for multiple simulations
  print("MSE is: ", mse, "vs. the variance: ", var)
  
  # Did we correctly conclude beta does not equal 0?
  for (elem in pval.test){
    if (!elem[2]){
      print(elem[1], "coefficient was not significant")
    }
    else{
      print(elem[1], "passes")
    }
...
# TO DO:
# create output of reg.tests results
# fix error
```
...
ind_poisson <- CIsim(n=20, samples=100, rdist=rbinom, args=list(size=1, prob=.7), estimand = .7, method = t.test, method.args=list(ci = "Plus4"))

ind_gamma <- CIsim(n = 20, samples=100, rdist = rgamma,
args = list(shape = shape, scale = scale),
method = t.test,
verbose = FALSE, estimand = mu)
head(ind_gamma)

histogram(~estimate, data = ind_gamma)

histogram(~estimate, data = ind_poisson)

xYplot(Cbind(estimate, lower, upper) ~ sample, data = ind_gamma, par.settings = col.mosaic(), groups = cover)
