...
multicollinearity <- function(n, func, bool = FALSE){
  ...
  x <- runif(n, 0, 10)
  x2_adding <- x + rnorm(n) #x2 is a linear combination of x1 and some number
  x2_mult <-x*rnorm(n)
...
From Melissa. Might want to add if we are having a hard time determining if a sample is correlated:
```{r}
